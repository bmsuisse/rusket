...
def evaluate_ndcg(algo, testset, trainset, n=10):
    ...
    # For Surprise (predicts scalars)
        ...
        for uid in valid_users:
            ...
            try:
                inner_u = trainset.to_inner_uid(uid)
                train_items = set([trainset.to_raw_iid(j) for j, r in trainset.ur[inner_u]])
            except:
                train_items = set()
...
def main():
    ...
    trainset, testset = split.train_test_split(data, test_size=.2, random_state=42)
    ...
    algo_implicit = implicit.bpr.BayesianPersonalizedRanking(
        factors=64, iterations=50, learning_rate=0.01, regularization=0.01, random_state=42
    )
