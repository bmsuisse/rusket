...
def make_chart(results: list[tuple]) -> go.Figure:
    ...
    fig = make_subplots(
        rows=2, cols=1,
        subplot_titles=("Wall-clock time vs rows processed", "Itemsets found vs rows processed"),
    ...
    fig.add_trace(go.Bar(
        name="add_chunk()", x=rows_m, y=add_t,
        marker_color=colors["add"], opacity=0.85,
        text=[f"{v:.1f}s" for v in add_t], textposition="inside",
        width=0.35,
    ), row=1, col=1)

    fig.add_trace(go.Bar(
        name="mine()", x=rows_m, y=mine_t,
        marker_color=colors["mine"], opacity=0.85,
        text=[f"{v:.1f}s" for v in mine_t], textposition="inside",
        width=0.35,
        base=add_t,
    ), row=1, col=1)

    fig.add_trace(go.Scatter(
        name="Total", x=rows_m, y=total_t,
    ...
    # -- Itemsets chart --
    fig.add_trace(go.Scatter(
        name="Itemsets found", x=rows_m, y=itemsets,
        mode="lines+markers",
