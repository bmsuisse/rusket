...
def bench_fpgrowth(rng: np.random.Generator, n_rows: int, n_txns: int, n_items: int,
                   min_support: float, max_len: int) -> Result:
...
def bench_fpminer(rng: np.random.Generator, n_rows: int, n_txns: int, n_items: int,
                  min_support: float, max_len: int, chunk_size: int = 10_000_000) -> Result:
...
def print_table(results: list[Result]) -> None:
    ...
    for r in results:
        ...
        print(f"{r.label:<10} {r.rows:>12,} {r.n_txns:>10,} {r.n_items:>8,} "
