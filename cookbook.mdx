...
freq_pairs = FPGrowth.from_transactions(
    df_long,
    transaction_col="receipt_id",
    item_col="product",
    min_support=0.02,
...
All miner classes accept long-format transaction data directly via `from_transactions`.
...
fi = FPGrowth(baskets, min_support=0.5, use_colnames=True).mine()
```
...
print(patterns_df[["support", "sequence"]].sort_values("support", ascending=False).head(10))
```
...
fi_pl = AutoMiner.from_transactions(
    df_pl,
    transaction_col="order_id",
    item_col="product_id",
    min_support=0.05,
