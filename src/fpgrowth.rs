...
impl FlatResults {
    ...
    pub fn append(&mut self, other: &FlatResults) {
    ...
    pub fn into_tuple(self) -> (Vec<u64>, Vec<u32>, Vec<u32>) {
        (self.supports, self.offsets, self.items)
    }
}

pub(crate) fn fpg_step(tree: &FPTree, minsup: u64, max_len: Option<usize>, results: &mut FlatResults) {
        ...
        for size in 1..size_remain {
            ...
            for combo in combinations(&local_ids, size) {
                let support = combo
                    .iter()
                    .map(|&local_id| {
                        tree.nodes[tree.item_nodes[local_id as usize][0] as usize].count
                    })
                    .min()
                    .unwrap_or(0);
        ...
        if use_par {
            let sub_results: Vec<FlatResults> = (0..num_items as u32)
                ...
                .map(|local_id| {
                    ...
                    fpg_step(&cond_tree, minsup, max_len, &mut sub);
                    sub
                })
                .collect();
            ...
            for local_id in (0..num_items as u32).rev() {
                ...
                fpg_step(&cond_tree, minsup, max_len, results);
            }
        }
    }
}
