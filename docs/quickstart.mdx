...
Most real-world data comes as long-format order lines (one row per product per order). `FPGrowth.from_transactions` converts that in one call:
...
<Tip>
You can manually select `Eclat` for sparse data (which is typical for large SKU catalogs) or use `FPGrowth` for dense data.
</Tip>
