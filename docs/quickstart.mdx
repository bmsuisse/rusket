...
Most real-world data comes as long-format order lines (one row per product per order). `AutoMiner.from_transactions` converts that in one call:
...
model = Eclat.from_transactions(orders, transaction_col="receipt_id", item_col="product", min_support=0.4)
freq = model.mine(use_colnames=True)
print(freq)  # identical output to fpgrowth
```

<Tip>
`AutoMiner` handles this automatically: it picks `Eclat` for sparse data (density < 0.15, typical for large SKU catalogs) and `FPGrowth` for dense data.
</Tip>
...
If you already have integer arrays from a data warehouse query, just pass them:
