# Serving Models with Vector Databases

A standard pattern for production recommender systems is to decouple **training** (which happens offline or periodically) from **serving** (which must respond to live user requests in milliseconds).

`rusket` supports exporting the latent factors (embeddings) from trained models directly into pandas DataFrames, enabling trivial ingestion into Vector Databases like **LanceDB**, **FAISS**, **Pinecone**, or **Qdrant**.

Below is a complete example showing how to train a model, export it, and serve it instantly with [LanceDB](https://lancedb.github.io/lancedb/).
...
Currently, you can export embeddings from any model that supports latent factors, including:
- `ALS`
- `BPR`
- `SVD`
- `LightGCN`
- `EASE` (item vectors only)
