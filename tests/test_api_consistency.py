...
import rusket
from rusket.model import ImplicitRecommender, Miner, RuleMinerMixin, SequentialRecommender
...
class TestClassHierarchy:
    ...
    def test_implicit_recommender_base(self, cls: type) -> None:
        assert issubclass(cls, ImplicitRecommender), f"{cls.__name__} must inherit ImplicitRecommender"
    ...
    def test_sequential_recommender_base(self, cls: type) -> None:
        assert issubclass(cls, SequentialRecommender), f"{cls.__name__} must inherit SequentialRecommender"
...
STOCHASTIC_MODELS = [cls for cls in ALL_RECOMMENDERS + STANDALONE if cls not in (rusket.EASE, rusket.ItemKNN)]
...
class TestInitSignature:
    ...
    def test_no_random_state(self, cls: type) -> None:
        sig = inspect.signature(cls.__init__)
        assert "random_state" not in sig.parameters, f"{cls.__name__}.__init__ uses 'random_state' — rename to 'seed'"

    @pytest.mark.parametrize("cls", STOCHASTIC_MODELS, ids=lambda c: c.__name__)
    def test_seed_param_exists(self, cls: type) -> None:
        sig = inspect.signature(cls.__init__)
        assert "seed" in sig.parameters, f"{cls.__name__}.__init__ must have a 'seed' parameter"
...
class TestRepr:
    ...
    def test_recommender_has_repr(self, cls: type) -> None:
        ...
        assert "__repr__" in cls.__dict__, f"{cls.__name__} must define __repr__"
...
class TestFromTransactions:
    ...
    def test_has_from_transactions(self, cls: type) -> None:
        assert hasattr(cls, "from_transactions"), f"{cls.__name__} must have from_transactions"
...
class TestRecommenderMethods:
    ...
    def test_has_recommend_items(self, cls: type) -> None:
        assert hasattr(cls, "recommend_items"), f"{cls.__name__} must have recommend_items"
    ...
    def test_recommend_items_signature(self, cls: type) -> None:
        ...
        assert "user_id" in params or len(params) >= 2, f"{cls.__name__}.recommend_items must accept user_id"
...
class TestMinerMethods:
    ...
    def test_has_association_rules(self, cls: type) -> None:
        assert hasattr(cls, "association_rules"), f"{cls.__name__} must have association_rules()"

    @pytest.mark.parametrize("cls", RULE_MINERS, ids=lambda c: c.__name__)
    def test_has_recommend_for_cart(self, cls: type) -> None:
        assert hasattr(cls, "recommend_for_cart"), f"{cls.__name__} must have recommend_for_cart()"
    ...
    def test_deprecated_recommend_items_alias(self, cls: type) -> None:
        ...
        assert hasattr(cls, "recommend_items"), f"{cls.__name__} must keep backward-compat recommend_items alias"
...
class TestDocstrings:
    ...
    def test_recommend_items_docstring_style(self, cls: type) -> None:
        ...
        assert "Args:" not in doc, f"{cls.__name__}.recommend_items uses Google-style docstring — use NumPy-style"
...
class TestExcludeSeen:
    ...
    def test_has_exclude_seen(self, cls: type) -> None:
        sig = inspect.signature(cls.recommend_items)
        assert "exclude_seen" in sig.parameters, f"{cls.__name__}.recommend_items must have exclude_seen parameter"
