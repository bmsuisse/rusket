...
def _generate_interactions(
    n_users: int, n_items: int, avg: int, seed: int = 42
) -> pd.DataFrame:
...
def large_als() -> ALS:
    ...
    return ALS(factors=64, iterations=3, seed=77).from_transactions(
        df, user_col="user_id", item_col="item_id"
    ).fit()
...
def test_bench_full_pipeline_500x1k(
    benchmark, small_als: ALS, small_bpr: BPR
) -> None:
...
@pytest.mark.benchmark(group="pipeline_full")
def test_bench_full_pipeline_with_filter(
    benchmark, small_als: ALS, small_bpr: BPR
) -> None:
